...
from models.knn import train as knn
from models.randomforests import train as randomforests
# from models.voting import train as voting
...
def compare_models(test_user=None, fp_flag=False):
    msg = str(time.time()) + ": Experimenting with models"
    print(msg)
    logging.info(msg)
    print("---------------------------------------------------")
    logging.info("---------------------------------------------------")
    ...
    ann_model_thread.join()
    randomforests_model_thread.join()
    knn_model_thread.join()
    extra_trees_model_thread.join()
...
def with_preprocessing_and_feature_extraction(test_user=None):
    ...
    result = compare_models(fp_flag='fp', test_user=test_user)
    msg = str(time.time()) + ": Experiment result is: " + str(result)
    print(msg)
    logging.info(msg)
