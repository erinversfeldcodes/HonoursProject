    ...
    for weight in weights_options:
        for neighbour in range(2, 10):
            for leaves in range(3, 50):
                ...
                # document results
                msg = str(time.time()) + ': Trained a KNN with the parameters' + str(("Weight: " + str(weight), "num neighbours: " + str(neighbour), "leaves: " + str(leaves) + " and it produced an accuracy score of " + str(knn_accuracy)))
                logging.info(msg)
                ...
                if knn_accuracy > max_accuracy:
                    ...
                    classifier_obj = knn
                    msg = str(time.time()) + ': The best KNN thus far is one trained with the parameters' + str(best_params) + ' which produced an accuracy score of: ' + str(max_accuracy)
                    logging.info(msg)

    msg = str(time.time()) + ': The best KNN was one trained with the parameters ' + str(best_params) + ' which produced an accuracy score of: ' + str(max_accuracy)
    print(msg)
    logging.info(msg)
