                ...
                for hidden_layer_sizes in range(1, 20):
                    ...
                    try:
                        ...
                        if mlp_accuracy > max_accuracy:
                            ...
                            best_algorithm = "MLP"
                            best_options = {"Layer sizes": hidden_layer_sizes, "Activation": activation,
                                            "Solver": solver,
                                            "Learning rate": learning_rate}

                    except ValueError:
                        print("X_train: " + str(list(x_train)))
                        print("Y_train: " + str(list(y_train)))

    msg = str(time.time()) + ': The best ANN was a ' + str(best_algorithm) + ' trained with the parameters' + str(
        best_options) + ' which produced an accuracy score of: ' + str(max_accuracy)
    print(msg)
    logging.info(msg)
    return [max_accuracy, best_algorithm, best_options]
