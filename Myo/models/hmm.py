...
def train(x_train, x_test, y_train, y_test, train_lengths, test_lengths, sensor_data, return_values):
    ...
    default_model.fit(x_train, y_train, train_lengths)
    max_accuracy = default_model.score(x_test, y_test, test_lengths)
    best_params = 'Default'
    method_params_found = 'Default'
    y_score = default_model.predict(x_test, lengths=test_lengths)
